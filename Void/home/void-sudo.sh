#!/bin/bash
set -euo pipefail
sudo mv -i ignore.conf /etc/xbps.d/
sudo xbps-install -Suy
sudo xbps-install -S xorg xorg-server-devel nvidia gvim pipewire alsa-utils alsa-pipewire noto-fonts-emoji noto-fonts-ttf-extra dejavu-fonts-ttf elogind dbus-elogind dbus-elogind-x11 dbus-elogind-libs polkit polkit-gnome wireplumber libjack-pipewire neofetch rxvt-unicode base-devel libX11-devel libXft-devel libXinerama-devel chrony git curl wget nitrogen scrot firefox firefox-i18n-en-US firefox-i18n-ar xdg-utils pcmanfm file-roller pavucontrol neofetch htop font-firacode breeze-cursors lxappearance gvfs gvfs-mtp mtpfs gvfs-gphoto2 ntfs-3g dialog mtools smartmontools qt5ct qbittorrent slock xss-lock rsync mpv mpc mpd ncmpcpp ranger ueberzug zsh zsh-completions dconf-editor galculator-gtk3 ufw gparted gnome-disk-utility papirus-icon-theme breeze-icons kvantum picom amiri-font font-sil-scheherazade font-sil-lateef NetworkManager socklog-void torbrowser-launcher rofi dunst nerd-fonts yaru obfs4proxy numlockx clipmenu qt5-styleplugins xtools leafpad zathura-pdf-mupdf redshift sxiv xclip figlet figlet-fonts lolcat-c alacrity bottom dust yt-dlp bat lsd ffmpeg conky fribidi-devel
